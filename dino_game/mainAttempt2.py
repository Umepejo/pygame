...
def _move_char(flip, x, y, anim_num, m_display, bg, idle):
...
while run == True:
    ...
    if cur_dir == "right":
        char = _move_char(False, xpos, ypos, a_num, dis, bg_img, False)
        last_dir = "right"
        a_num += 1
    elif cur_dir == "left":
        char = _move_char(True, xpos, ypos, a_num, dis, bg_img, False)
        last_dir = "left"
        a_num += 1
    else:
        a_num = 0
        if last_dir == "right":
            char = _move_char(False, xpos, ypos, a_num, dis, bg_img, True)
        elif last_dir == "left":
            char = _move_char(True, xpos, ypos, a_num, dis, bg_img, True)
    ...
    if leftOrRight == True:
        xpos = char[0]
        ypos = char[1]
        dinoRect = (xpos, ypos, d_width, d_height)
